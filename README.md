...
- generate_header.py: Script that automatically converts exported weights, biases, and scaling factors into a C-code header.
- nn_data.h: Generated C-code header file, containing the network configuration/data.
- main.c: STM32 firmware. Receives noisy signal windows via UART, executes the fixed-point inference of the neural network, applies the conventional algorithms, and measures CPU execution cycles.
- inference.c: Contains the fixed-point logic for the hardware neural network inference.
- filters.c: Implementation of the Kalman algorithm, Wiener algorithm (FIR-filter), and the IIR (low-pass) filter (EMA (Exponential Moving Average))
...
## Evaluated metrics
After hardware inference, the system prints an evaluation-report for every filtering method for each noise scenario:
